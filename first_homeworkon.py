...
import matplotlib.pyplot # включаем библиотеку для построения графиков, задавая ее краткое имя как "mpp"
...
if __name__=='__main__': # не до конца понял, по какому принципу выполняется эта операция (откуда взялась переменная __name__ и почему она по умолчанию равна '__main__')
    ...
    matplotlib.pyplot.plot(       # начинаем задавать параметры графика
    ...
    matplotlib.pyplot.show() # команда отображения функции в виде графика
